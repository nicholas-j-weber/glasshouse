...
// §9.2: sheet data, including version history, persisted in IndexedDB
// (single local user, no accounts). Addendum S: multiple independent sheets
// now coexist in the same database — still no sync, no collaboration.

export interface HeadRecord {
  // Addendum S: no longer a fixed "head" singleton — one row per sheet,
  // keyed by sheetId.
  id: string;
  versionId: string;
}
...
export class ContextSheetDB extends Dexie {
  ...
  constructor(name = "context-sheets") {
    ...
    // Addendum S: adds multi-sheet support (sheets, messages) and scopes
    // versions/head by sheetId. Per the addendum's "Migration: none"
    // decision, existing versions/head rows predate sheetId entirely and
    ...
    // Addendum V: adds the usage table. Purely additive — no existing
