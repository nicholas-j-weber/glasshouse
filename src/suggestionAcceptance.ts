...
// a fallback entry wasn't genuinely proposed by the
...
// category direct manual edits already use, since the *decision*
// to add it was the user's even though the *text* wasn't authored by the
// model.
//
// autoApplied distinguishes a version created without a manual
...
// edit_memory targeting an id in neither pool (the
...
export function resolveContentChange(
  ...
  switch (suggestion.type) {
    case "new_memory": {
      // an ordinary memory always targets the global pool.
    ...
    case "compress_conversation": {
      ...
      //
      // matches "summary" turnIds too, not just
      ...
      // where they were, just dimmed (inactive stays visible, excluded
    ...
    case "edit_memory": {
      // could target either pool — the "emergent
      ...
      // ordinary memory left in local storage ("no migration"
