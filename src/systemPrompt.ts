...
function modePreamble(mode: CallMode): string {
  return mode === "chat" ? CHAT_PREAMBLE : SHEET_EDITOR_PREAMBLE;
}

export function buildSystemPrompt(sheet: Sheet, mode: CallMode): string {
  return [modePreamble(mode), serializeSheet(sheet), SUGGESTION_INSTRUCTIONS].join("\n\n");
}
